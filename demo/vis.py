...
def calculate_angle(pointA, pointB, pointC):
    ...
    vectorBC = pointB - pointC
    cosine_angle = np.dot(vectorAB, vectorBC) / (np.linalg.norm(vectorAB) * np.linalg.norm(vectorBC))
    angle = np.degrees(np.arccos(cosine_angle))
    return angle

def img2video_with_legality(video_path, output_dir, legality_results):
    cap = cv2.VideoCapture(video_path)
    fps = int(cap.get(cv2.CAP_PROP_FPS)) + 5
    ...
    videoWrite = cv2.VideoWriter(output_dir + video_name + '_with_legality.mp4', fourcc, fps, size) 

    for i, name in enumerate(names):
        img = cv2.imread(name)
        if i < len(legality_results):  # Ensure legality_results is not accessed out of bounds
            legality_text = legality_results[i]
            # Choose text color based on legality
            text_color = (0, 255, 0) if legality_text == "Legal" else (0, 0, 255)
            cv2.putText(img, legality_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, text_color, 2, cv2.LINE_AA)
        videoWrite.write(img)
...
def get_pose3D(video_path, output_dir):
    ...
    print('\nGenerating demo...')
    legality_results = []  # Initialize legality_results list
    for i in tqdm(range(len(image_2d_dir))):
        # Your existing code here...

        # Calculate the angle of the elbow
        keypoints_file = os.path.join(output_dir + 'input_2D/', 'keypoints.npz')
        keypoints_data = np.load(keypoints_file, allow_pickle=True)['reconstruction']
        keypoints_frame = keypoints_data[0][i]  # Get keypoints for the current frame

        # Extract the relevant points for the elbow angle calculation (assuming COCO format)
        pointA = keypoints_frame[6]  # Left shoulder
        pointB = keypoints_frame[7]  # Left elbow
        pointC = keypoints_frame[8]  # Left wrist

        # Calculate the angle of the elbow
        angle_elbow = calculate_angle(pointA, pointB, pointC)

        # Determine if the angle is less than 165 degrees
        if angle_elbow < 160:
            legality_results.append(f"{angle_elbow} degrees: Not Legal")
        else:
            legality_results.append(f"{angle_elbow} degrees: Legal")

        
    print("Legality results for the angle of the elbow:")
    for result in legality_results:
        print(result)
    ...
    img2video_with_legality(video_path, output_dir, legality_results)
